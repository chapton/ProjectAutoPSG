...
fsize = np.array((6.0,5.0))

#
#with open("results_dataset_edfx-sample.pkl", "rb") as f:
...
pkl = pickle.load(open('results_dataset_edfx-sample.pkl', 'rb'))
t_label = ['W', 'N1', 'N2', 'N3', 'R']
for key in pkl:
    pprint(pkl)
    confmat = [x[4] for x in pkl[key]]
    tools.plot_confusion_matrix ('dataset_'+ key + '.png',np.mean(confmat,0), t_label, perc=True, cmap=cmap, title=key)
