...
if __name__ == '__main__':
    ...
    if not os.path.isfile(os.path.join(datadir, 'sleepdata.h5')):
        ...
        channels = {'EEG':'C4-M1', 'EMG':'CHIN1-CHIN2', 'EOG':'EOG-R-M1'} # set channels that are used
        ...
        sleep.load( channels = channels, references = references, verbose="Warning", resampling = True, dtype=np.float32)
...
#%%
    ...
    rnn = {'model':models.bi_lstm, 'layers': ['fc1'],  'seqlen':6,
           'epochs': 250,  'batch_size': 256,  'stop_after':15, 'balanced':False}
    print(rnn)
    model = models.cnn3adam_filter_morel2
    results = keras_utils.cv (data, target, groups, model, rnn=rnn, name='KCMH_CNN3ADAM',
                             epochs=epochs, folds=10, batch_size=batch_size, counter=0,
