...
import sleeploader
import matplotlib; matplotlib.rcParams['figure.figsize'] = (10.24, 7.68)
np.random.seed(42)
...
#    testsets.remove(trainset)
    ...
    cnn, lstm = keras_utils.train_models(data, target, groups, model=models.cnn3adam_filter_morel2)
    cnn.save(os.path.join('.','weights','traintransfer_cnn_{}_{}'.format(trainset,normmode)), include_optimizer=False)
    lstm.save(os.path.join('.','weights','traintransfer_lstm_{}_{}'.format(trainset,normmode)), include_optimizer=False)
    results = {}
    for dataset in testsets:
        gc.collect()
        data,target,groups = load_data(dataset, norm=normmode)
        results[dataset] = keras_utils.test_data_cnn_rnn(data,target,groups,cnn,lstm)
        cnn_acc, cnn_f1, rnn_acc, rnn_f1, confmat, preds = results[dataset]
    ...
    with open('results_transfer_{}_{}'.format(trainset, normmode), 'wb') as f:
