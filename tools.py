...
def plot_results_per_patient(predictions, targets, groups, title='Results per Patient', fname='results_pp.png', plothyp=False):
    ...
    for ID in IDs:
        ...
        if plothyp:
            ...
            ax = plt.subplot(111)
            plot_hypnogram(y_pred, c='orangered', title='Ground Truth', ax1=ax)
            plot_hypnogram(y_true, c='royalblue', title='Prediction', ax1=ax, linewidth=1.9)
            plt.legend(['Human Scorer', 'CNN+LSTM'], loc='lower right')
            # plt.savefig('./plots/hypnogram_truth.pdf')
            plt.savefig('./plots/hypnogram_prediction_{}_{}.pdf'.format(acc, ID))
    if fname != '':plt.figure()
...
def plot_hypnogram(stages, labels=None, title='', ax1=None, **kwargs):
    if labels is None:
        if np.max(stages)==4:
            #print('assuming 0=W, 1=N1, 2=N2, 3=N3, 4=R')
            labels = ['W', 'N1', 'N2', 'N3', 'R']
        if np.max(stages)==5:
    ...
    for i in np.arange(len(stages)):
        ...
        if labels_dict[s]=='R': p = -1
        if labels_dict[s]=='N1':  p = -2
        if labels_dict[s]=='N2':  p = -3
        if labels_dict[s]=='N3': p = -4
    ...
    ax1.plot(x,y, **kwargs)
    plt.yticks([0,-1,-2,-3,-4,-5], ['W', 'R', 'N1', 'N2', 'N3'])
